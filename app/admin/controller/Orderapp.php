...
class Orderapp extends  TenantController
{
    ...
    public function index(Request $request){
        ...
        $builder->addTableColumn(['title' => '订单号', 'field' => 'order_no', 'minWidth' => 150])
            ...
            ->addRightButton('delete', ['text' => '退款', 'href' => url('refundPost', ['id' => '__data_id__'])]);
        return $builder->show();
    }
    ...
    public function refundPost(Request $request){
        $id = input('id', 0);
        if(! $order = $this->model->where('order_status', \app\common\constant\Payment::PAID)->find($id)){
            return $this->error('此订单无法退款！');
        }
        $res = PayService::wxRefund([
            'order_no' => $order['order_no'],
            'refund_no' => build_order_no('refund'),
            'total_fee' => $order['amount'],
            'refund_fee' => $order['amount']
        ]);
        if($res['code']){
            $order['refund_id'] = $res['data']['refund_id'];
            OrderService::afterRefund($order);
