...
namespace ky\EasyCps\Vip\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use ky\EasyCps\Vip\Osp\Context\InvocationContextFactory;
...
class OspStub
{
    ...
    {
        \ky\EasyCps\Vip\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][" . $this->serviceName . "-" . $this->version . "-" . $method . "]");
        ...
        , $timestamp, $version, $request, $appSecret)
    {
        ...
        return \ky\EasyCps\Vip\Osp\Util\HmacUtil::hmac($sign, $appSecret);
    }
    ...
    {
        if (!$restAddr) throw new \ky\EasyCps\Vip\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
    ...
    {
        $ctx = InvocationContextFactory::getInstance();
        $oprot = new \ky\EasyCps\Vip\Osp\Protocol\JSONProtocol(new \ky\EasyCps\Vip\Osp\Buffer\MemoryBuffer());
        $args->write($oprot);
        ...
        $client = new \ky\EasyCps\Vip\Osp\Http\HttpClient($scheme, $host, $port);
        $request = array();

        $appKey = $ctx->getAppKey();
        if ($appKey == null) throw new \ky\EasyCps\Vip\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");

        $appSecret = $ctx->getAppSecret();
        if ($appSecret == null) throw new \ky\EasyCps\Vip\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
        ...
        $timestamp = round(\ky\EasyCps\Vip\Osp\Util\TimeUtil::currentTimeMillis() / 1000);
        ...
        try {
            ...
            throw new \ky\EasyCps\Vip\Osp\Exception\OspException(\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()), $e);
        }
    ...
    public function receive_base($result)
    {
        ...
        if (\ky\EasyCps\Vip\Osp\Base\OspStub::$RETURN_NULL != $response) {
            $iprot = new \ky\EasyCps\Vip\Osp\Protocol\JSONProtocol(new \ky\EasyCps\Vip\Osp\Buffer\MemoryBuffer());
            ...
            if (!"0" == $returnCode) {
                ...
                throw new \ky\EasyCps\Vip\Osp\Exception\OspException($returnMessage, $returnCode);
            }
