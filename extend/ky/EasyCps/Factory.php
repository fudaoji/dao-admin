...
use ky\EasyCps\TaoBao\Application as TaoBao;
use ky\EasyCps\PinDuoDuo\Application as PinDuoDuo;
use ky\EasyCps\JingDong\Application as JingDong;
use ky\EasyCps\Vip\Application as Vip;
use ky\EasyCps\SuNing\Application as SuNing;
use ky\EasyCps\Vip\Osp\Context\InvocationContext;
...
class Factory
{
    ...
    protected function getConfig(string $name, array $config)
    {
        ...
        if ($name == "jingdong") {
            ...
            return $this->arrayOnly($config, ['app_key', 'app_secret']);
        }
        if ($name == "vip") {
            if (!array_key_exists('app_key', $config) || !array_key_exists('app_secret', $config)) {
                throw new \InvalidArgumentException('The vip client requires app_key and app_secret.');
            }
            return $this->arrayOnly($config, ['app_key', 'app_secret', 'access_token', 'format']);
        }
        if ($name == "suning") {
            if (!array_key_exists('app_key', $config) || !array_key_exists('app_secret', $config)) {
     ...
     * @param string $name
     * @param array $config
     * @return JingDong|PinDuoDuo|SuNing|TaoBao
     */
    ...
    private function arrayOnly(array $array, $columns)
    {
        if (is_array($columns)) {
            $data = [];
