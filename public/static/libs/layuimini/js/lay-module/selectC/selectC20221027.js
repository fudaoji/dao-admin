...
layui.define(['jquery', 'laydate','form'],function(exports){
	...
	obj.prototype.init = function(){
		...
		var formElem = '<form id="selectC-form" class="layui-form"><div class="layui-form-item">CONTENT<div class="layui-inline"><button type="button" class="layui-btn" id="selectC-search-btn">查询</button>';
		if(config.reset){
			formElem += '<button type="button" class="layui-btn layui-btn-primary" id="selectC-reset">清空</button>';
		}
		formElem += '</div></div></form>';
		...
		for (var i = 0; i < n; i++) {
			html +=	'<div class="layui-input-inline" style="width:100px;">'+
			      	'<div id="lay-unselect'+i+'" lay-index="'+ i +'" class="layui-unselect layui-form-select">'+
						'<div class="layui-select-title">'+
							'<input id="layui-select-title" type="text" placeholder="'+config.title+'" value="" readonly="" class="layui-input layui-unselect">'+
							'<i class="layui-edge"/>'+
						'</div>'+
			...
			for (var j = 0; j < o.length; j++) {
				var selectedClass = o[j].name == config.defaultSelected ? 'layui-this' : '';
				html +=	'<dd lay-value="'+o[j].name+'" class="'+selectedClass+'">'+o[j].elemName+'</dd>';
			...
			html +=	'<div class="layui-input-inline" id="inputBox'+i+'">'+
						'<input type="text" value="'+config.value+'" id="disabled" autocomplete="off" class="layui-input" disabled>'+
					'</div>';
		}
		html += '</div>';
		if(config.form) {
			html = formElem.replace('CONTENT', html);
	...
	obj.prototype.createInput = function(n,id){
		...
		for (var l = 0; l < o.length; l++) {
			if(n == o[l].name){
				...
				//日期框需要初始化
				if(o[l].type == 'date' || o[l].type == 'datetime'){
